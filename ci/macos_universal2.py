...
def _run_cmd(cmd):
    print(f'RUN {cmd}')
    rv = subprocess.run(cmd, shell=False, capture_output=True, encoding='utf-8')
    print(rv.stdout)
    if bool(rv.stderr):
        print(f'STDERR:\n{rv.stderr}')
    if rv.returncode:
...
def is_universal2_file(path):
    rv = _run_cmd(['lipo', '-info', path])
    return not ('Non-fat' in rv.stdout)


def is_universal2(package):
    rv = _run_cmd(['pip', 'show', '-f', package])
    header = True
    for line in rv.stdout.split('\n'):
        line = line.strip()
    ...
    for package in packages:
        ...
        if name in delocate_packages:
            ...
            try:
                ...
                f3 = _run_pip_download(['pip', 'download', '--only-binary', ':all:', '--platform', platform0, f'{name}=={version}'])
            except Exception:
                f3 = None
            if f3 is None or '-none-any' in f3:
                print(f'PATCH {name}')
                f1 = _run_pip_download(['pip', 'download', '--only-binary', ':all:', '--platform', platform1, f'{name}=={version}'])
                f2 = _run_pip_download(['pip', 'download', '--only-binary', ':all:', '--platform', platform2, f'{name}=={version}'])
                if 'x86_64' not in f1 or 'arm64' not in f2:
                    raise RuntimeError('Could not find matching files')
                _run_cmd(['delocate-fuse', f1, f2, '-w', '.'])
                f3 = f2.replace('arm64', 'universal2')
                os.rename(f1, f3)
            _run_cmd(['pip', 'install', '--force-reinstall', '-f', '.', f3])
        else:
            print(f'SKIP {name} already universal2')

    _run_cmd(['pip', 'uninstall', '-y'] + delocate_packages)
    return 0
